...
public class SampleApp_Vantiv_Tandem {
	...
	public ArrayOfBankcardService SI;
	public ArrayOfElectronicCheckingService BCSI;
	...
	public boolean SaveApplicationConfiguration(ConfigurationValues _CV) {
		...
		try {
			...
			AD.setApplicationLocation(ApplicationLocation.ON_PREMISES);
			AD.setApplicationName("OptiTab");
	...
	public boolean RetrieveServiceInformation(ConfigurationValues _CV) {
		...
		try {
			...
			if(SI.getBankcardService() != null && SI.getBankcardService().size() > 0)
			{
				...
				for (BankcardService b : SI.getBankcardService())
				{
	...
	//ElectronicChecking Services
			BCSI = GSIR.getElectronicCheckingServices();
			// NOTE: At this point you can either iterate through the services
			// in SI or in the case of our example just use the first one.
			if(BCSI.getElectronicCheckingService() != null && BCSI.getElectronicCheckingService().size() > 0)
			{
				boolean ECKFirstService = true;
				System.out.println("*** Key Information about the Electronic Checking Service(s) ***\r\n");
				//Use the following to use the first service in the scenario for first time through sample.
				for (ElectronicCheckingService e : BCSI.getElectronicCheckingService())
				{
	...
	public boolean SaveMerchantProfile(ConfigurationValues _CV) {
		...
		try {
			...
			BTDD.setEntryMode(com.ipcommerce.schemas.cws.v2_0.serviceinformation.EntryMode.TRACK_DATA_FROM_MSR);
			MPTD.setBankcardTransactionDataDefaults(BTDD);
			BankcardMerchantData BCMD = new BankcardMerchantData();
			BCMD.setAgentChain("2323");
			BCMD.setClientNumber("2323");
			BCMD.setSIC("0022");
			BCMD.setTerminalId("001");
			BCMD.setIndustryType(com.ipcommerce.schemas.cws.v2_0.serviceinformation.IndustryType.RESTAURANT);
		...
		*/ 
		...
		try {
			...
			if(_CV.ServiceToProcessAs == "BankcardService" && _CV._bcs != null){
				...
				if (_CV._bcs.getOperations().isAuthAndCapture())
					if(!AuthorizeAndCapture(_CV))return false;
...
/// The following are helper methods
    ...
    public Addendum SetUsernameAndPassword(String username, String password)
    {
        ...
        creds.getString().add("<LogonCreds><UserId>"+username+"</UserId><Password>"+password+"</Password></LogonCreds>");
